...
def default_hps():
  return HyperParams(num_steps=2000, # train model for 2000 steps.
                     max_seq_len=80, # train on sequences of 100
                     input_seq_width=32+4,    # width of our data (32 + 3 actions)
...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    self.cell = cell
    self.input_single = tf.placeholder(dtype=tf.float32, shape=[None, INWIDTH])
    self.state_c = tf.placeholder(dtype=tf.float32, shape=[None, hps.rnn_size])
    self.state_h = tf.placeholder(dtype=tf.float32, shape=[None, hps.rnn_size])
    self.output_batch, self.state_tuples = self.cell(self.input_single, (self.state_c, self.state_h))

    self.sequence_lengths = LENGTH # assume every sample has same length.
    self.input_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len - 1, INWIDTH])
    self.output_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len - 1, OUTWIDTH])
    self.output_tq = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len - 1, 1])
    ...
    with tf.variable_scope('RNN'):
      ...
      #tq_w = tf.get_variable("tq_w", [NOUT, 1])
      #tq_b = tf.get_variable("tq_b", [1])
    ...
    output = tf.reshape(output, [-1, NOUT])
    self.final_state = last_state   
    self.output_batch = tf.reshape(self.output_batch, [-1, hps.rnn_size])
    self.output_batch = tf.nn.xw_plus_b(self.output_batch, output_w, output_b)
    self.output_batch = tf.reshape(self.output_batch, [-1, NOUT])
    
    #self.output_batch = tf.nn.xw_plus_b(self.output_batch, tq_w, tq_b)
    #self.output_batch = tf.reshape(self.output_batch, [-1, 1])
    
    #self.tq_pred = tf.nn.xw_plus_b(output, tq_w, tq_b)
    #self.tq_pred = tf.reshape(self.tq_pred, [-1, 1])

    # reshape target data so that it is compatible with prediction shape
    target_data = tf.reshape(self.output_x,[-1, NOUT])
    #output_tq = tf.reshape(self.output_tq, [-1, 1])
    self.change_loss = tf.reduce_mean(tf.square(target_data - output))
    #self.pred_loss = tf.reduce_mean(tf.square(self.tq_pred - output_tq))
    self.cost = self.change_loss# + self.pred_loss
